...
fun main() {
    ...
    val possibleGameIds = input.lines().map { line ->
        val gameSplit = line.split(":")
        val gameNumber = gameSplit[0].split(" ").last().toInt()

        val matches = regex.findAll(gameSplit[1])
        var maxRed = 0
        var maxGreen = 0
        var maxBlue = 0
        ...
        matches.forEach { match ->
            ...
            when (val color = match.groupValues[2]) {
                "red" -> maxRed = max(maxRed, quantity)
                "green" -> maxGreen = max(maxGreen, quantity)
                "blue" -> maxBlue = max(maxBlue, quantity)
        ...
        if (
            maxRed <= redThreshold &&
            maxGreen <= greenThreshold &&
            maxBlue <= blueThreshold
        ) {
            gameNumber
        } else {
            0
