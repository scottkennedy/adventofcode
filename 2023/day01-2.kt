fun main() {
    val values = input.lines().map { line ->
        val firstRegex = "^.*?(${allDigits.keys.joinToString("|")}).*".toRegex()
        val lastRegex = "^.*(${allDigits.keys.joinToString("|")}).*?".toRegex()
        val firstMatch = firstRegex.matchEntire(line)!!
        val lastMatch = lastRegex.matchEntire(line)!!
        val first = firstMatch.groupValues[1]
        val last = lastMatch.groupValues[1]
        val firstDigit = allDigits[first]!!
