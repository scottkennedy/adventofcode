fun main() {
    ...
    input.lines().mapIndexed { rowIndex, line ->
        line.mapIndexed { colIndex, char ->
            if (char == '*') {
                val gearRatios = getGearRatios(rowIndex, colIndex)
...
private fun getGearRatios(rowIndex: Int, colIndex: Int): List<Int> {
    ...
    if (rowIndex >= 1) {
        if (lines[rowIndex - 1][colIndex].isDigit()) {
            gearRatios += getGearRatio(rowIndex - 1, colIndex)
        }
    }

    // only if 1 up was nothing
    if (rowIndex >= 1 && !lines[rowIndex - 1][colIndex].isDigit()) {
        // 1 up, 1 left
        if (colIndex >= 1 && lines[rowIndex - 1][colIndex - 1].isDigit()) {
            gearRatios += getGearRatio(rowIndex - 1, colIndex - 1)
        }

        // 1 up, 1 right
        if (colIndex < lines[rowIndex - 1].lastIndex && input.lines()[rowIndex - 1][colIndex + 1].isDigit()) {
            gearRatios += getGearRatio(rowIndex - 1, colIndex + 1)
        }
    }
    ...
    // 1 left
    if (colIndex >= 1) {
        if (lines[rowIndex][colIndex - 1].isDigit()) {
            gearRatios += getGearRatio(rowIndex, colIndex - 1)
        }
    }

    // 1 right
    if (colIndex < lines[rowIndex].lastIndex) {
        if (lines[rowIndex][colIndex + 1].isDigit()) {
            gearRatios += getGearRatio(rowIndex, colIndex + 1)
        }
    }
    ...
    // 1 down
    if (rowIndex < lines.lastIndex) {
        if (lines[rowIndex + 1][colIndex].isDigit()) {
            gearRatios += getGearRatio(rowIndex + 1, colIndex)
        }
    }

    // only if 1 down was nothing
    if (rowIndex < lines.lastIndex && !lines[rowIndex + 1][colIndex].isDigit()) {
        // 1 down, 1 left
        if (colIndex >= 1 && lines[rowIndex + 1][colIndex - 1].isDigit()) {
            gearRatios += getGearRatio(rowIndex + 1, colIndex - 1)
        }
...
private fun getGearRatio(rowIndex: Int, colIndex: Int): Int {
    var start = colIndex
    var end = colIndex

    while (start > 0 && input.lines()[rowIndex][start - 1].isDigit()) {
        start--
    }

    while (end < input.lines()[rowIndex].lastIndex && input.lines()[rowIndex][end + 1].isDigit()) {
        end++
    }

    val partNumber = input.lines()[rowIndex].substring(start, end + 1)
